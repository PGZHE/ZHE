...
<BODY>
	...
	<DIV id=regLogin class=wrap>
		<DIV class=dialog>
			...
			<DIV class=box>
				<FORM id=add_action action=update2?hid=${xx.hid} method="post">
					<DIV class=infos>
						<TABLE class=field>
							<TBODY>
								<TR>
									...
									<TD><INPUT id=add_action_title class=text type=text
										name=hname value="${xx.hname}"></TD>
								</TR>
								<TR>
									<TD class=field>户 型：</TD>
									<TD><SELECT class=text name=tid>
									<c:forEach items="${type}" var="t">
									<OPTION  value="${t.tid}" ${xx.tid eq t.tid?'selected':''}>${t.tname}</OPTION>
									</c:forEach>
										</SELECT>
									</TD>
								</TR>
								<TR>
									...
									<TD><INPUT id=add_action_floorage class=text type=text
										name=hmianji value="${xx.hmianji}"></TD>
								</TR>
								<TR>
									<TD class=field>价 格：</TD>
									<TD><INPUT id=add_action_price class=text type=text
										name=hqian value="${xx.hqian}"></TD>
								</TR>
								<TR>
									<TD class=field>房产证日期：</TD>
									<TD><INPUT class=text type=text name=shijian value="${xx.shijian}"></TD>
								</TR>
								<TR>
									...
									<OPTION value="${q.did}" ${xx.did eq q.did?'selected':''}>${q.dname}</OPTION>
									</c:forEach>
									</SELECT>
									 街：<SELECT class=text name=sid>
									<c:forEach items="${jie}" var="j">
									 <OPTION value="${j.sid}" ${xx.sid eq j.sid?'selected':''}>${j.sname}</OPTION>
									 </c:forEach>
									 </SELECT>
									</TD>
								</TR>
							...
							<td><input type="text" class="text" name="point.y" /></td>
								...
								<TR>
									...
									<TD><INPUT id=add_action_contact class=text type=text
										name=dianhua value="${xx.dianhua}"></TD>
								</TR>
								<TR>
									<TD class=field>详细信息：</TD>
									<TD><TEXTAREA name=xiangxi>${xx.xiangxi }</TEXTAREA></TD>
								</TR>
							</TBODY>
						</TABLE>
						<DIV class=buttons>
